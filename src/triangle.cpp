                       ...
                       Triangle::Intersection& intersection)
{
  ...
  float divisor = DotProduct(edge1, p);
  ...
  float b1 = invDivisor * DotProduct(t, p);
  if (b1 < 0.0 || b1 > 1.0)
    return false;

  // compute barycentric coordnate b2
  Vector q = CrossProduct(t, edge1);
  float b2 = invDivisor * DotProduct(ray.GetDirection(), q);
  if (b2 < 0.0 || b1 + b2 > 1.0)
    return false;

  // compute distance from ray origin to intersection point
  float distance = invDivisor * DotProduct(edge2, q);
  if (distance < 0.0)
    return false;

  intersection.t = distance;
  intersection.epsilon = 1e-3f * distance;
  intersection.b1 = b1;
  intersection.b2 = b2;
  return true;
}
